...
% work on multiple branches.
    ...
    if makeGitCommits
        ...
        %% set sport mode, BMS tests updated
        disp('Checkout bBattMgmt')
            [~,cmdout] = system("git checkout bBattMgmt");
            disp(cmdout)
        disp('Update BMS Tests')
            updateBMSTests('sportMode')
        disp('Committing Battery sportMode tests to bBattMgmt')
            commitMessage = 'git commit -am "setup bBattMgmt with tests for sport mode"';
...
%% Helper Functions
    ...
    % in latest release and R2022b release
        ...
        vcuMdl = 'VCU_2EMEV';
        load_system(vcuMdl)
        blkname = ['VCU_2EMEV/ctrl_powertrain/Energy Management/' ...
            'Control Domain /Series Regen Braking/RegenStrt'];
    ...
    % This function modified the BMS model regen operation table, and saves it
        ...
        bmsMdl = 'BMS_Software';
        load_system(bmsMdl)
        blkname = 'BMS_Software/CurrPowerLimCalc/sportModeGain';
        set_param(blkname,"Gain",num2str(currentGain));
